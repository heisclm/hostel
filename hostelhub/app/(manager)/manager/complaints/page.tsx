    ...
    return (
      <>
        ...
        <motion.div
          ...
          className="fixed right-0 top-0 bottom-0 w-full sm:w-125 bg-white shadow-2xl z-50 flex items-center justify-center"
        >
                  ...
                  <div className="space-y-1.5">
                    ...
                    {complaint.user.studentProfile?.studentId && (
                      <p className="text-sm text-slate-600 flex items-center gap-2">
                        ...
                        Student ID: {complaint.user.studentProfile.studentId}
                      </p>
                    )}
                  </div>
                )}
...
export default function ManagerComplaintsPage() {
  ...
  const [managerHostels, setManagerHostels] = useState<ManagerHostel[]>([]);
  ...
  useEffect(() => {
    const fetchManagerHostels = async () => {
      try {
        const response = await complaintService.getManagerHostels();
        if (response.success && response.data.length > 0) {
          setManagerHostels(response.data);
          setSelectedHostelId(response.data[0].id);
        }
      } catch (error) {
        console.error("Failed to fetch manager hostels:", error);
  ...
  const fetchComplaints = useCallback(async () => {
    ...
    try {
      const [complaintsRes, statsRes] = await Promise.all([
        complaintService.getHostelComplaints(selectedHostelId, {
          status: statusFilter !== "all" ? statusFilter : undefined,
          priority: priorityFilter !== "all" ? priorityFilter : undefined,
          search: debouncedSearch || undefined,
        }),
        complaintService.getHostelComplaintStats(selectedHostelId),
      ]);
  ...
  }, [selectedHostelId, statusFilter, priorityFilter, debouncedSearch]);
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      if (selectedHostelId) {
        const statsRes =
          await complaintService.getHostelComplaintStats(selectedHostelId);
        if (statsRes.success) {
          setStats(statsRes.data.stats);
        }
      }
    ...
    return (
      <div className="bg-white rounded-xl border border-slate-200 py-16 text-center">
        ...
        <p className="text-slate-500">
          You don&apos;t have any hostels assigned to manage.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Manage and respond to tenant complaints
          </p>
        </div>
        <div className="flex items-center gap-3">
          {managerHostels.length > 1 && (
            <div className="relative">
              <select
                value={selectedHostelId}
                onChange={(e) => setSelectedHostelId(e.target.value)}
                className="appearance-none pl-4 pr-10 py-2.5 text-sm border border-slate-200 rounded-lg bg-white cursor-pointer font-medium"
              >
      ...
      {isLoadingComplaints ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 py-16 text-center">
          ...
          <p className="text-slate-500">
            All caught up! No complaints match your filters.
          </p>
        </div>
      )}
